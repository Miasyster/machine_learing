...
from utils.task_helpers import load_config, create_default_args
...
with TaskGroup('health_checks', dag=dag) as health_checks_group:
    ...
    # 设置TaskGroup内部依赖
    system_health_check >> check_data_paths
...
with TaskGroup('quality_monitoring', dag=dag) as quality_monitoring_group:
    ...
    # 设置TaskGroup内部依赖
    data_quality_monitoring >> generate_html_report
...
with TaskGroup('alert_processing', dag=dag) as alert_processing_group:
    ...
    # 设置告警处理组内部依赖
    check_alert_status >> send_summary_email
...
with TaskGroup('maintenance', dag=dag) as maintenance_group:
    ...
    # 设置维护组内部依赖
    cleanup_old_data >> update_monitoring_stats
...
# 完成任务
complete_monitoring = EmptyOperator(
...
# TaskGroup内部依赖已在各自的TaskGroup定义中设置